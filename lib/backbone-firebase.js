...
(function(exports, undefined){
   ...
   _.extend(BackboneFirebase.prototype, Backbone.Events, {
      ...
      _bindEvents: function() {
         ...
         _.each(events, function(f, event) {
               var fn = disposableEvent(this, this.reference, event, f);
      ...
      _add: function(pushed_model) {
         ...
         if( notEcho('before_remote_create', model, this.syncManager) ) {
            this.trigger('before_remote_create', model);
   ...
   BackboneFirebase.defaultEvents = {
      ...
      child_changed: function(pushed_model) {
         ...
         if (model) {
            if( notEcho('before_remote_update', model, this.syncManager) ) {
               this.trigger('before_remote_update', model);
      ...
      child_removed: function(pushed_model) {
         ...
         if (model && notEcho('before_remote_destroy', model, this.syncManager)) {
            console.log('deleting'); //debug
            this.trigger('before_remote_destroy', model);
   ...
   Backbone.sync = function(method, model, options) {
      ...
      var ref = getFirebaseRef(options.urlPrefix, path);
   ...
   function notEcho(action, model, syncManager) {
      return !syncManager || !syncManager.isEcho(action, model);
   }
